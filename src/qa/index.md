...
可以通过 `resetAuth` 接口重置认证信息：

1. 在环境变量中设置 `RESET_KEY`（任意字符串，建议足够复杂），然后重启服务
2. 浏览器访问 `https://你的域名/api/auth/resetAuth?key=你设置的RESET_KEY`
3. 成功后认证信息被清除（其他安全设置如域名白名单、审核配置不受影响），可以直接进入管理端重新设置密码
4. **用完后务必删除或更换 `RESET_KEY` 环境变量**

## Q: 如何清除已设置的密码（改为无密码访问）？

进入 **系统设置 -> 安全设置 -> 认证管理**，在密码输入框旁边勾选 **清除密码** 复选框，然后点击保存即可。清除管理端密码时会同时清除管理员用户名。
