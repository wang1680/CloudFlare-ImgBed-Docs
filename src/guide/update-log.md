...
Add Features:
- 管理端文件列表排序功能增强，支持按时间、大小、文件 ID 和文件名进行升序或降序排列，记忆当前浏览器的排序偏好，并通过组合图标直观展示排序字段与方向
- 系统设置 → 系统状态新增动态问候与运行概览区域，根据当前时间显示问候语、本地时钟和时区，并依据索引接口响应、文件数及上传渠道数生成中英文运行状态摘要
- 网页设置新增“默认语言”下拉选项，支持“简体中文”和 English，默认为简体中文；首次进入页面时按“用户持久化语言 → 网页默认语言 → 简体中文”的优先级初始化语言
- 上传设置的渠道类型新增“全部 / 已配置”筛选，默认仅展示已配置且当前可用的渠道；当前选择不可用且存在其他可用渠道时，会自动切换到首个已配置渠道
...
Fix Bugs:
- 修复上传项在确认失败、关闭自动重试或达到最大自动重试次数后，进度条仍显示上传中动画的问题；失败项现在会显示静态错误状态并保留手动重试入口，手动重试时重置进度和自动重试计数后重新上传
...
- 修复深色模式 Logo 悬停效果未按页面主题正确生效，以及多条全局消息因固定顶部位置而相互重叠的问题
- 修复上传页标题蜡笔书写动画底部被裁切的问题，并调整标题内容层级与响应式留白，确保悬停切换完整显示

Optimization:
- 提高亮色模式下下拉菜单、选择器、上传渠道筛选器和系统设置标签页折叠按钮的 hover 背景对比度，并恢复选择器展开时当前选中项的 hover 背景，使交互状态更清晰一致
- 优化管理端顶栏页面切换器的深色 hover 对比度、当前项与其他选项间距及高亮退出动画，避免高亮在鼠标移出时滑回首项
- 统一默认上传与粘贴链接卡片的虚线边框，并增强深浅色模式下的中性 hover 边框、透明度和柔和阴影反馈
- 提高亮色模式通用玻璃卡片、弹窗和下拉菜单的边框对比度，同时拉开常态与 hover 边框的视觉层级
- 统一管理端排序菜单、系统设置下拉选择器和上传设置渠道筛选器的弹窗间距、圆角、选中状态及深浅色 hover 效果，避免选项背景贴满弹窗宽度
- 系统设置 → 系统状态顶部的三个概览卡片新增 hover 悬浮效果，通过轻微上移、阴影与边框过渡增强交互反馈，并适配系统的减少动态效果偏好
- 上传渠道分布和文件状态分布的条状标签新增 hover 反馈，通过轻微上浮以及背景、边框和阴影变化提升交互辨识度，并适配深浅色模式和减少动态效果偏好
- 调整网页设置项目顺序，将“默认语言”置于“网站图标”后，并将“启用壁纸”更名为“启用背景图”并移至“背景切换间隔”前
- 补充“默认 URL 前缀”提示，说明其适用于 CDN 加速等场景，并提醒确认自定义前缀可访问
...
- 优化可用高度较小的桌面视口布局，压缩上传页标题与上传区域上方留白，并收紧上传列表工具栏尺寸；同时预加载标题字体以改善首屏显示
