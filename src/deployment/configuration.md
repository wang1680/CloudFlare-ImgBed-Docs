...
- **默认语言**：用户没有持久化语言设置时使用的网页默认语言，可选“简体中文”和 English，默认为简体中文。进入页面时按“用户持久化语言 → 网页默认语言 → 简体中文”的优先级初始化语言。
...
- **启用背景图**：控制所有页面是否加载背景图，默认开启。关闭并保存后，页面刷新时将使用纯色背景；具体背景图来源仍由客户端设置和管理端设置中的背景图选项决定。
- **背景切换间隔**：背景图轮播间隔，单位为毫秒。默认 `3000`，例如希望 10 秒切换一次可填写 `10000`。
- **背景图透明度**：背景图显示透明度，填写 `0-1` 之间的小数。默认 `1`，例如 `0.8`。
- **默认 URL 前缀**：用于生成全局默认访问链接和上传 API 成功响应中的 `publicUrl`，仅影响页面展示和接口返回的链接，不会改变文件的实际访问路径。适用于 CDN 加速等场景，请确保自定义前缀可以正常访问。示例：`https://img.example.com/file/`；留空时使用当前站点域名。
